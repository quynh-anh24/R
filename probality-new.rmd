...
---
Cuốn sách giới thiệu về vai trò quan trọng của xác suất không chỉ trong trò chơi may rủi mà còn trong phân tích dữ liệu. Xác suất không chỉ là khái niệm trò chơi mà còn là công cụ cần thiết để hiểu và mô tả các sự kiện trong thế giới thực khi xử lý dữ liệu. Sử dụng ngôn ngữ lập trình R, cuốn sách giúp người đọc hiểu sâu hơn về nguyên tắc xác suất trong thực tế thông qua việc chạy mã và thấy kết quả trực quan. Tuy nhiên, sách không áp dụng ngay các khái niệm xác suất vào dữ liệu thực tế, mà sẽ trình bày mối liên hệ giữa lý thuyết xác suất và dữ liệu thực tế trong phần tiếp theo của cuốn sách.
3.Xác suất rời rạc
Chúng tôi bắt đầu bằng cách đề cập đến một số nguyên tắc cơ bản liên quan đến dữ liệu phân loại. Tập hợp con của xác suất được gọi là xác suất rời rạc .Nó sẽ giúp chúng ta hiểu lý thuyết xác suất mà sau này chúng ta sẽ giới thiệu cho dữ liệu số và dữ liệu liên tục, vốn phổ biến hơn nhiều trong các ứng dụng khoa học dữ liệu.
3.1 Tần số tương đối
Từ "xác suất" thường được sử dụng trong ngôn ngữ hàng ngày. Việc trả lời các câu hỏi về xác suất thường khó, nếu không nói là không thể.Nên ta có ví dụ 
Nếu tôi có 2 hạt màu đỏ và 3 hạt màu xanh trong một cái hũ (hầu hết các sách về xác suất sử dụng thuật ngữ cổ điển này, chúng ta cũng vậy) và tôi chọn ngẫu nhiên một hạt, xác suất chọn một hạt màu đỏ là bao nhiêu? Trực giác của chúng ta cho biết câu trả lời là 2/5 hoặc 40%. Một định nghĩa chính xác có thể được đưa ra bằng cách lưu ý rằng có năm kết quả có thể xảy ra và trong đó có hai kết quả thỏa mãn điều kiện cần thiết cho sự kiện "chọn một hạt màu đỏ". Vì mỗi trong số năm kết quả đều có cùng cơ hội xảy ra, chúng ta kết luận rằng xác suất là 0,4 cho màu đỏ và 0,6 cho màu xanh.
Một cách cụ thể hơn để nghĩ về xác suất của một sự kiện là tỷ lệ số lần sự kiện xảy ra khi chúng ta lặp lại thực nghiệm vô hạn số lần, độc lập và dưới điều kiện giống nhau.

Tóm lại, xác suất có thể được hiểu là tỷ lệ số lần sự kiện xảy ra khi thực hiện thử nghiệm lặp lại vô hạn lần, độc lập và dưới cùng điều kiện.
3.2 Ký hiệu
Chúng tôi sử dụng ký hiệu Pr(A)để biểu thị xác suất của sự kiện A
...
Trong các ứng dụng khoa học dữ liệu, chúng ta thường xử lý các biến liên tục. Những sự kiện này thường sẽ là những câu như “người này có cao hơn 6 feet không”. Trong trường hợp này, chúng ta viết các sự kiện ở dạng toán học hơn:X>=6 
...
Một ví dụ là samplehàm trong R. Chúng tôi minh họa cách sử dụng nó trong đoạn mã bên dưới. Đầu tiên, chúng ta sử dụng hàm rep để tạo bình:
```{r}
...
set.seed(1986) 
```
Trong cuốn sách, chúng tôi sử dụng các trình tạo số ngẫu nhiên để minh họa các kết quả. Điều này cho thấy kết quả có thể thay đổi tình cờ, và phiên bản cố định của cuốn sách có thể hiển thị kết quả khác nhau khi thực hiện mã. Điều này là do tính ngẫu nhiên của kết quả và chúng có thể thay đổi theo thời gian. Tuy nhiên, để đảm bảo sự nhất quán, bạn có thể đặt hạt giống tạo số ngẫu nhiên của R thành một số cụ thể. Một cách phổ biến là sử dụng ngày tháng năm, ví dụ như 1986/12/20, để đảm bảo mỗi lần chạy mã sẽ cho ra kết quả giống nhau.
cách thiết lập hạt giống bằng cách xem tài liệu:
```{r}
?set.seed
```
...
3.7.1 Quy tắc nhân
Nếu chúng ta muốn biết xác suất của hai sự kiện, hãy nóiA Và B, xảy ra, chúng ta có thể sử dụng quy tắc nhân:
...
Vì vậy, trong trò chơi Blackjack, để tính cơ hội nhận được số 21 bằng cách rút một quân Át và sau đó là một quân bài mặt, chúng ta tính xác suất của quân Át đầu tiên và nhân với xác suất rút được một quân bài mặt hoặc một quân 10 cho trước đầu tiên là quân Át:1/13x16/51~~0.025
...
paste cũng hoạt động trên các cặp vectơ thực hiện thao tác theo từng phần tử:
...
#> [1] 0.377
```
CDF là một phiên bản của eCDF gán các xác suất lý thuyết cho mỗi a thay vì tỷ lệ được tính toán từ dữ liệu. Mặc dù, như chúng tôi vừa chứng minh, các tỷ lệ được tính toán từ dữ liệu có thể được sử dụng để xác định xác suất cho một biến ngẫu nhiên. Cụ thể là CDF cho kết quả ngẫu nhiên X định nghĩa, cho bất kỳ số nào a, xác suất quan sát được một giá trị lớn hơn a.
     $$ F(a) = \mbox{Pr}(X \leq a) $$
Khi CDF được xác định, chúng ta có thể sử dụng nó để tính xác suất của bất kỳ tập hợp con giá trị nào. Chẳng hạn, xác suất để một học sinh nằm giữa chiều cao a và chiều cao b là:
$$
...
Một kết quả toán học thực sự rất hữu ích trong thực tế là đối với hầu hết các CDF, chúng ta có thể định nghĩa một hàm, gọi nó là f(x), điều đó cho phép chúng ta xây dựng CDF bằng Giải tích, như thế này:

$$
F(b) - F(a) = \int_a^b f(x)\,dx
$$ 
f(x) được gọi là hàm mật độ xác suất . Trực giác cho thấy ngay cả đối với các kết quả liên tục, chúng ta có thể xác định các khoảng nhỏ, gần như nhỏ bằng điểm, có xác suất dương. Nếu chúng ta coi kích thước của các khoảng này là đáy của hình chữ nhật thì hàm mật độ xác suất f xác định chiều cao của hình chữ nhật sao cho tổng diện tích của các hình chữ nhật này gần đúng với xác suất F(b) - F(a). Tổng này có thể được viết dưới dạng tổng Reimann được tính gần đúng bằng tích phân:
```{r echo=FALSE}
...
4.4 Mật độ xác suất
Đối với phân phối phân loại, chúng ta có thể xác định xác suất của một danh mục. Ví dụ như tung xúc xắc, hãy gọi nó là X, có thể là 1, 2, 3, 4, 5 hoặc 6. Xác suất của 4 được xác định như sau:
$$
\mbox{Pr}(X=4) = 1/6
$$
...
$$
Mặc dù đối với phân phối liên tục xác suất của một giá trị duy nhất Pr(X=x) không được định nghĩa, có một định nghĩa lý thuyết có cách giải thích tương tự. Mật độ xác suất tại x được định nghĩa là hàm f(a) như vậy mà:
$$
...
f(x) như một đường cong mà diện tích bên dưới đường cong đó đạt đến giá trị a, cho bạn xác suất Pr(X<=a).
...
4.5 Monte Carlo
R cung cấp các chức năng để tạo ra các kết quả có phân phối chuẩn. Cụ thể, hàm rnorm nhận ba đối số: kích thước, trung bình (mặc định là 0) và độ lệch chuẩn (mặc định là 1) và tạo ra các số ngẫu nhiên. Dưới đây là ví dụ về cách chúng tôi có thể tạo dữ liệu giống như chiều cao được báo cáo của chúng tôi:
```{r}
n <- length(x)
m <- mean(x)
s <- sd(x)
...
6. Để xem phép toán giải thích tại sao câu trả lời cho câu hỏi 3, 4 và 5 giống nhau, giả sử chúng ta có một biến ngẫu nhiên có giá trị trung bình m và sai số chuẩn s. Giả sử chúng ta hỏi xác suất của X nhỏ hơn hoặc bằng a. Hãy nhớ rằng, theo định nghĩa,
a là (a - m)/s độ lệch chuẩn s cách xa mức trung bình m. Xác suất là:
$$
\mbox{Pr}(X \leq a)
$$
Bây giờ chúng ta trừ μ sang cả hai bên rồi chia cả hai bên cho σ:
$$
\mbox{Pr}\left(\frac{X-m}{s} \leq \frac{a-m}{s} \right)
$$
Đại lượng bên trái là một biến ngẫu nhiên chuẩn tắc tiêu chuẩn. Nó có trung bình là 0 và sai số chuẩn là 1. Chúng ta sẽ gọi nó là Z:
$$
\mbox{Pr}\left(Z \leq \frac{a-m}{s} \right)
$$
Vì vậy, bất kể đơn vị là gì, xác suất của X <= a giống như xác suất của một biến thông thường tiêu chuẩn nhỏ hơn (a - m)/s. Nếu mulà giá trị trung bình và sigma lỗi tiêu chuẩn, mã R nào sau đây sẽ cho chúng ta câu trả lời đúng trong mọi tình huống:
...
8. Sự phân bố điểm IQ gần như được phân phối chuẩn. Giá trị trung bình là 100 và độ lệch chuẩn là 15. Giả sử bạn muốn biết sự phân bổ chỉ số IQ cao nhất trong tất cả các lớp tốt nghiệp nếu mỗi lớp có 10.000 người sinh ra trong khu học chánh của bạn. Chạy mô phỏng Monte Carlo với B=1000 việc tạo ra 10.000 điểm IQ và giữ mức cao nhất. Tạo một biểu đồ.
...
```
Đây X là một biến ngẫu nhiên: mỗi lần chúng ta chọn một hạt mới thì kết quả sẽ thay đổi ngẫu nhiên. Xem bên dưới:
```{r}
...
Chúng ta sẽ định nghĩa một biến ngẫu nhiên S điều đó sẽ đại diện cho tổng số tiền thắng cược của sòng bạc. Hãy bắt đầu bằng cách xây dựng chiếc bình. Một bánh xe roulette có 18 ô màu đỏ, 18 ô màu đen và 2 ô màu xanh lá cây. Vì vậy, việc chơi một màu trong một trò chơi roulette tương đương với việc rút ra từ chiếc bình này:
```{r}
color <- rep(c("Black", "Red", "Green"), c(18, 18, 2))
```
1.000 kết quả từ 1.000 người chơi là những kết quả rút ra độc lập từ chiếc bình này. Nếu màu đỏ xuất hiện, con bạc thắng và sòng bạc thua một đô la, vì vậy chúng ta rút -1$. Ngược lại, sòng bạc thắng được 1 đô la và chúng tôi rút được 1 đô la. Để xây dựng biến ngẫu nhiên của chúng tôi S, chúng ta có thể sử dụng mã này:
```{r}
n <- 1000
...
Nếu bạn chạy đoạn mã trên, bạn sẽ thấy rằng S thay đổi mỗi lần. Tất nhiên là vì S là một biến ngẫu nhiên Phân bố xác suất của một biến ngẫu nhiên cho chúng ta biết xác suất giá trị quan sát rơi vào bất kỳ khoảng thời gian nào. Vì vậy, ví dụ, nếu chúng ta muốn biết xác suất để chúng ta mất tiền, chúng ta đang hỏi xác suất để S đang trong khoảng thời gian.

Lưu ý rằng nếu chúng ta có thể định nghĩa hàm phân phối tích lũy F(a) = Pr(S<=a), thì chúng ta sẽ có thể trả lời bất kỳ câu hỏi nào liên quan đến xác suất của các sự kiện được xác định bởi biến ngẫu nhiên của chúng ta S, bao gồm cả sự kiện S < 0. Chúng tôi gọi đây là F hàm phân phối của biến ngẫu nhiên .

Chúng ta có thể ước tính hàm phân phối cho biến ngẫu nhiên S bằng cách sử dụng mô phỏng Monte Carlo để tạo ra nhiều cách thể hiện biến ngẫu nhiên. Với mã này, chúng tôi tiến hành thử nghiệm cho 1.000 người chơi roulette, lặp đi lặp lại, cụ thể là B = 10,000 lần:
```{r}
n <- 1000
B <- 10000
...
```
Bây giờ chúng ta có thể hỏi như sau: trong các mô phỏng của mình, chúng ta có thường xuyên nhận được tổng nhỏ hơn hoặc bằng không a?
```{r}
mean(S <= a)
```
Đây sẽ là một xấp xỉ rất tốt của F(a) và chúng ta có thể dễ dàng trả lời câu hỏi của sòng bạc: khả năng chúng ta sẽ mất tiền là bao nhiêu? Chúng ta có thể thấy nó khá thấp:
```{r}
mean(S < 0)
#> [1] 0.0456
```
Chúng ta có thể hình dung sự phân bố của S bằng cách tạo biểu đồ hiển thị xác suất F(b) - F(a) trong vài khoảng thời gian (a,b]:
...
S. Chúng tôi sẽ nói nhiều hơn về những điều này trong phần tiếp theo.
Lý thuyết thống kê cung cấp một cách để rút ra sự phân bố của các biến ngẫu nhiên được xác định là các lần rút ngẫu nhiên độc lập từ một chiếc bình. Cụ thể, trong ví dụ trên, chúng ta có thể chỉ ra rằng (S + n)/2 tuân theo phân phối nhị thức. Do đó, chúng ta không cần phải chạy mô phỏng Monte Carlo để biết phân bố xác suất của S. Chúng tôi đã làm điều này cho mục đích minh họa.
Chúng ta có thể sử dụng hàm dbinomvà pbinomtính toán xác suất một cách chính xác. Ví dụ, để tính toán Pr(S < 0) chúng tôi chú ý điều đó:

và chúng ta có thể sử dụng pbinom để tính toán
```{r}
n <- 1000
pbinom(n/2, size = n, prob = 10/19)
#> [1] 0.0511
```
Bởi vì đây là hàm xác suất rời rạc nên để có được Pr(S < 0) còn hơn là Pr(S <= 0), chúng tôi viết:
```{r}
...
Trước khi tiếp tục, chúng ta hãy phân biệt và kết nối quan trọng giữa phân bố của danh sách các số và phân bố xác suất. Trong chương trực quan, chúng tôi đã mô tả cách thức bất kỳ danh sách số nào x1,...,xn có sự phân phối. Định nghĩa khá đơn giản. Chúng tôi xác định F(a) là hàm cho chúng ta biết tỷ lệ nào của danh sách nhỏ hơn hoặc bằng a. Bởi vì chúng là những tóm tắt hữu ích khi phân phối gần như bình thường nên chúng ta xác định độ lệch trung bình và độ lệch chuẩn. Chúng được xác định bằng một phép toán đơn giản của vectơ chứa danh sách các số x:
```{r}
m <- sum(x)/length(x)
s <- sqrt(sum((x - m)^2) / length(x))
```
Một biến ngẫu nhiên X có chức năng phân phối. Để xác định điều này, chúng ta không cần một danh sách các con số. Đó là một khái niệm lý thuyết. Trong trường hợp này, chúng tôi xác định phân phối là F(a) để trả lời câu hỏi: xác suất để X nhỏ hơn hoặc bằng a? Không có danh sách các số.

Tuy nhiên, nếu X được xác định bằng cách vẽ từ một chiếc bình có các số trong đó, khi đó sẽ có một danh sách: danh sách các số bên trong chiếc bình. Trong trường hợp này, phân bố của danh sách đó là phân bố xác suất của X còn giá trị trung bình và độ lệch chuẩn của danh sách đó là giá trị kỳ vọng và sai số chuẩn của biến ngẫu nhiên.

Một cách khác để suy nghĩ về nó mà không liên quan đến chiếc bình là chạy mô phỏng Monte Carlo và tạo ra một danh sách rất lớn các kết quả của X. Những kết quả này là một danh sách các con số. Sự phân bố của danh sách này sẽ gần đúng với phân bố xác suất của X. Danh sách càng dài thì xấp xỉ càng tốt. Độ lệch trung bình và độ lệch chuẩn của danh sách này sẽ xấp xỉ giá trị kỳ vọng và sai số chuẩn của biến ngẫu nhiên.

5.5 Ký hiệu cho các biến ngẫu nhiên
Trong sách giáo khoa thống kê, chữ in hoa được sử dụng để biểu thị các biến ngẫu nhiên và chúng tôi tuân theo quy ước này ở đây. Các chữ cái viết thường được sử dụng cho các giá trị được quan sát. Bạn sẽ thấy một số ký hiệu bao gồm cả hai. Ví dụ: bạn sẽ thấy các sự kiện được xác định là X<=x. Đây X là một biến ngẫu nhiên, làm cho nó trở thành một sự kiện ngẫu nhiên, và x là một giá trị tùy ý và không ngẫu nhiên. Ví dụ,X có thể đại diện cho số trên một cuộn súc sắc và x sẽ đại diện cho một giá trị thực tế mà chúng ta thấy 1, 2, 3, 4, 5 hoặc 6. Vì vậy, trong trường hợp này, xác suất của X=x là 1/6 bất kể giá trị quan sát được x. Ký hiệu này hơi lạ vì khi chúng ta đặt câu hỏi về xác suất, X không phải là đại lượng quan sát được. Thay vào đó, đó là một đại lượng ngẫu nhiên mà chúng ta sẽ thấy trong tương lai. Chúng ta có thể nói về những gì chúng ta mong đợi, những giá trị nào có thể xảy ra, nhưng không phải nó là gì. Nhưng một khi chúng ta có dữ liệu, chúng ta sẽ thấy sự hiện thực hóa X. Vì vậy, các nhà khoa học dữ liệu sẽ nói về những gì có thể xảy ra sau khi chúng ta thấy những gì thực sự đã xảy ra.
...
Khái niệm quan trọng đầu tiên cần tìm hiểu là giá trị kỳ vọng . Trong các sách thống kê người ta thường sử dụng chữ cái E như thế này:
$$\mbox{E}[X]$$
để biểu thị giá trị kỳ vọng của biến ngẫu nhiên X.
...
Đối với biến ngẫu nhiên rời rạc với các kết quả có thể xảy ra x1,...,xn giá trị mong đợi được xác định là
$$
\mbox{E}[X] = \sum_{i=1}^n x_i \,\mbox{Pr}(X = x_i)
$$
Nếu như X là một biến ngẫu nhiên liên tục, có khoảng giá trị
a ĐẾN b và hàm mật độ xác suất f(x), tổng này trở thành tích phân:
$$
\mbox{E}[X] = \int_a^b x f(x)
$$
Lưu ý rằng trong trường hợp chúng ta đang chọn các giá trị từ một un urn trong đó mỗi giá trị x(i) có cơ hội bình đẳng 1/n việc được chọn phương trình trên chỉ đơn giản là giá trị trung bình của xi(S)
$$
...
\mbox{E}[X] = (20 + -18)/38
$$
tức là khoảng 5 xu. Bạn có thể nghĩ rằng hơi phản trực giác khi nói rằng X thay đổi khoảng 0,05, khi giá trị duy nhất mà nó nhận là 1 và -1. Một cách để hiểu giá trị kỳ vọng trong bối cảnh này là nhận ra rằng nếu chúng ta chơi đi chơi lại trò chơi, sòng bạc sẽ thắng trung bình 5 xu mỗi trò chơi. Mô phỏng Monte Carlo xác nhận điều này:
```{r}
B <- 10^6
x <- sample(c(-1, 1), B, replace = TRUE, prob = c(9/19, 10/19))
mean(x)
```
Nói chung, nếu chiếc bình có hai kết quả có thể xảy ra, chẳng hạn a Và b, với tỷ lệ p Và 1 - p tương ứng, trung bình là:
$$\mbox{E}[X] = ap + b(1-p)$$
Để thấy điều này, hãy chú ý rằng nếu có n hạt trong bình, sau đó chúng ta có n p a cát n(1 - p)bs và vì trung bình là tổng,
n x a x p + n x b x (1 - p), chia cho tổng n, chúng ta nhận được rằng trung bình là ap + b(1 - p).

Bây giờ lý do chúng ta xác định giá trị kỳ vọng là vì định nghĩa toán học này hóa ra lại hữu ích trong việc tính gần đúng phân bố xác suất của tổng, sau đó hữu ích cho việc mô tả phân bố của các giá trị trung bình và tỷ lệ. Thực tế hữu ích đầu tiên là giá trị kỳ vọng của tổng số lần rút là số lần rút x trung bình cộng của các số trong bình.

Vì vậy, nếu 1.000 người chơi roulette, sòng bạc dự kiến ​​sẽ thắng trung bình khoảng 1.000 người.x 0,05 USD = 50 USD. Nhưng đây là một giá trị mong đợi. Một quan sát có thể khác với giá trị kỳ vọng như thế nào? Sòng bạc thực sự cần phải biết điều này. Phạm vi khả năng là gì? Nếu số âm quá có khả năng xảy ra, họ sẽ không cài đặt bánh xe roulette. Lý thuyết thống kê một lần nữa trả lời câu hỏi này. Sai số chuẩn (SE) cho chúng ta ý tưởng về độ lớn của biến thể xung quanh giá trị kỳ vọng. Trong sách thống kê, người ta thường sử dụng:
$$\mbox{E}[X] = ap + b(1-p)$$
để biểu thị sai số chuẩn của một biến ngẫu nhiên.

Đối với biến ngẫu nhiên rời rạc với các kết quả có thể xảy ra x1,...,xn lỗi tiêu chuẩn được định nghĩa là
$$
\mbox{SE}[X] = \sqrt{\sum_{i=1}^n \left(x_i - E[X]\right)^2 \,\mbox{Pr}(X = x_i)},
$$
mà bạn có thể coi là khoảng cách trung bình dự kiến của X
từ giá trị mong đợi.
...
$$
Lưu ý rằng trong trường hợp chúng ta đang chọn các giá trị từ một un urn trong đó mỗi giá trị xi có cơ hội bình đẳng 1/n việc được chọn phương trình trên chỉ đơn giản là độ lệch chuẩn của
xiS
$$
\mbox{SE}[X] = \sqrt{\frac{1}{n}\sum_{i=1}^n (x_i - \bar{x})^2} \mbox{ with } \bar{x} =  \frac{1}{n}\sum_{i=1}^n x_i 
$$
Sử dụng định nghĩa về độ lệch chuẩn, bằng một chút toán học, chúng ta có thể rút ra rằng nếu một chiếc bình chứa hai giá trị a Và b với tỷ lệ p Và (1 - p), tương ứng, độ lệch chuẩn là:
$$\mid b - a \mid \sqrt{p(1-p)}.$$
Vì vậy, trong ví dụ roulette của chúng tôi, độ lệch chuẩn của các giá trị bên trong bình là: |1 - (-1)|sqrt(10/10 x 9/19) hoặc:
```{r}
2 * sqrt(90)/19
#> [1] 0.999
```
...
S <- replicate(B, roulette_winnings(n))
```
Định lý giới hạn trung tâm (CLT) cho chúng ta biết rằng tổng S được xấp xỉ bằng phân phối chuẩn. Sử dụng các công thức trên, chúng ta biết rằng giá trị kỳ vọng và sai số chuẩn là:
```{r}
n * (20 - 18)/38 
#> [1] 52.6
sqrt(n)*2*sqrt(90)/19 
...
Nếu X là những lần rút độc lập từ chiếc bình thì tất cả chúng đều có cùng giá trị kỳ vọng. Hãy gọi nó là μ và như vậy:
$$ 
...
Hệ quả của 3 và 4 là sai số chuẩn của giá trị trung bình của các lần rút độc lập từ cùng một chiếc bình là độ lệch chuẩn của chiếc bình đó chia cho căn bậc hai của n(số lần rút), gọi nó là σ:
$$
\begin{aligned}
...
\end{aligned}
$$
5. Nếu X là biến ngẫu nhiên có phân phối chuẩn thì nếu a Và b là các hằng số không ngẫu nhiên, aX + b cũng là biến ngẫu nhiên có phân phối chuẩn. Tất cả những gì chúng ta đang làm là thay đổi đơn vị của biến ngẫu nhiên bằng cách nhân với a, sau đó dịch chuyển tâm bằng b.

Lưu ý rằng sách giáo khoa thống kê sử dụng các chữ cái Hy Lạp μ Và σ để biểu thị giá trị kỳ vọng và sai số chuẩn tương ứng. Điều này là do μ là chữ cái Hy Lạp cho m, chữ cái đầu tiên của giá trị trung bình, là một thuật ngữ khác được sử dụng cho giá trị kỳ vọng. Tương tự,σ là chữ cái Hy Lạp cho s, chữ cái đầu tiên của lỗi tiêu chuẩn.

5.8.1 Luật số lớn
Một ý nghĩa quan trọng của kết quả 5 ở trên là sai số chuẩn của giá trị trung bình ngày càng nhỏ đi khi n ngày càng lớn hơn. Khi n là rất lớn thì sai số chuẩn thực tế là bằng 0 và giá trị trung bình của các lần rút hội tụ về giá trị trung bình của bình. Điều này được biết đến trong sách giáo khoa thống kê là quy luật số lớn hoặc quy luật trung bình.

5.9 Bài tập
1. Trong American Roulette bạn cũng có thể đặt cược vào màu xanh lá cây. Có 18 màu đỏ, 18 màu đen và 2 màu xanh lá cây (0 và 00). Cơ hội màu xanh lá cây xuất hiện là gì?

2. Khoản thanh toán cho chiến thắng trên sân cỏ là 17 đô la. Điều này có nghĩa là nếu bạn đặt cược một đô la và nó chuyển sang màu xanh lá cây, bạn sẽ nhận được 17 đô la. Tạo mô hình lấy mẫu sử dụng mẫu để mô phỏng biến ngẫu nhiên X cho tiền thắng cược của bạn. Gợi ý: xem ví dụ bên dưới để biết nó trông như thế nào khi đặt cược vào màu đỏ.
```{r}
...
4. Tính sai số chuẩn của X.

5. Bây giờ tạo một biến ngẫu nhiên S đó là tổng số tiền thắng của bạn sau khi đặt cược vào ô xanh 1000 lần. Gợi ý: thay đổi đối số sizevà replacetrong câu trả lời của bạn cho câu hỏi 2. Bắt đầu mã của bạn bằng cách đặt hạt giống thành 1 với set.seed(1).

6. Giá trị kỳ vọng của S?

7. Sai số chuẩn của S?

8. Xác suất để bạn thắng được tiền là bao nhiêu? Gợi ý: sử dụng CLT.

9. Tạo mô phỏng Monte Carlo tạo ra 1.000 kết quả S. Tính độ lệch trung bình và độ lệch chuẩn của danh sách kết quả để xác nhận kết quả của 6 và 7. Bắt đầu mã của bạn bằng cách đặt hạt giống thành 1 với set.seed(1).

10. Bây giờ hãy kiểm tra câu trả lời của bạn ở số 8 bằng kết quả Monte Carlo.

11. Kết quả Monte Carlo và xấp xỉ CLT gần giống nhau, nhưng không gần bằng nhau. Điều gì có thể giải thích cho điều này?

a. 1.000 mô phỏng là không đủ. Nếu chúng ta làm nhiều hơn, chúng sẽ khớp nhau.
b. CLT không hoạt động tốt khi xác suất thành công nhỏ. Trong trường hợp này, đó là ngày 19/1. Nếu chúng ta tăng số lần chơi roulette lên thì chúng sẽ phù hợp hơn.
c. Sự khác biệt nằm ở lỗi làm tròn.
d. CLT chỉ hoạt động ở mức trung bình.

12. Bây giờ tạo một biến ngẫu nhiên Y đó là số tiền thắng trung bình của bạn cho mỗi lần đặt cược sau khi chơi hết số tiền thắng sau khi đặt cược vào ô xanh 1.000 lần.

13. Giá trị kỳ vọng của Y?

14. Sai số chuẩn của Y?

15. Xác suất để bạn giành được chiến thắng dương trong mỗi trận đấu là bao nhiêu? Gợi ý: sử dụng CLT.

16. Tạo mô phỏng Monte Carlo tạo ra 2.500 kết quả Y. Tính độ lệch trung bình và độ lệch chuẩn của danh sách kết quả để xác nhận kết quả của 13 và 14. Bắt đầu mã của bạn bằng cách đặt hạt giống thành 1 với set.seed(1).

17. Bây giờ hãy so sánh câu trả lời của bạn với 15 bằng kết quả Monte Carlo.

18. Kết quả Monte Carlo và phép tính gần đúng CLT giờ đã gần hơn rất nhiều. Điều gì có thể giải thích cho điều này?

a. Bây giờ chúng ta đang tính số trung bình thay vì tính tổng.
b. 2.500 mô phỏng Monte Carlo không tốt hơn 1.000.
c. CLT hoạt động tốt hơn khi cỡ mẫu lớn hơn. Chúng tôi đã tăng từ 1.000 lên 2.500.
d. Nó không gần hơn. Sự khác biệt nằm ở lỗi làm tròn.

19. Các phiên bản phức tạp hơn của mô hình lấy mẫu mà chúng ta đã thảo luận cũng được các ngân hàng sử dụng để quyết định lãi suất và các công ty bảo hiểm để quyết định phí bảo hiểm. Để hiểu điều này, giả sử bạn điều hành một ngân hàng nhỏ có lịch sử xác định những chủ sở hữu nhà tiềm năng có thể tin cậy để thanh toán. Trên thực tế, trong lịch sử, trong một năm nhất định, chỉ có 2% khách hàng của bạn vỡ nợ, nghĩa là họ không trả lại số tiền bạn đã cho họ vay. Giả sử ngân hàng của bạn sẽ cho vay $n=$1.000 với số tiền là 180.000 USD trong năm nay. Ngoài ra, sau khi cộng tất cả các chi phí, giả sử ngân hàng của bạn thua lỗ l=$200.000 cho mỗi lần tịch thu tài sản thế chấp. Để đơn giản, chúng tôi giả định điều này bao gồm tất cả chi phí hoạt động. Lợi nhuận dự kiến là bao nhiêu S cho ngân hàng của bạn trong kịch bản này?

20. Lưu ý rằng tổng tổn thất được xác định bằng tổng cuối cùng trong bài tập trước là một biến ngẫu nhiên. Mỗi khi bạn chạy mã mô hình lấy mẫu, bạn sẽ nhận được số lượng người mặc định khác nhau dẫn đến mức lỗ khác nhau. Mã hóa mô hình lấy mẫu cho biến ngẫu nhiên đại diện cho lợi nhuận ngân hàng của bạn S theo kịch bản được mô tả ở phần 19.

21. Bài tập trước chứng minh rằng nếu bạn chỉ cho mọi người vay tiền không lãi suất, bạn sẽ mất tiền do không trả được nợ 2%. Mặc dù bạn biết 2% khách hàng của mình có thể sẽ vỡ nợ nhưng bạn không biết đó là những khách hàng nào nên bạn không thể xóa họ. Tuy nhiên, bằng cách tính phí cho mọi người thêm một chút tiền lãi, bạn có thể bù đắp khoản lỗ phát sinh do 2% đó và cũng trang trải được chi phí hoạt động của mình. Số lượng x bạn có phải tính phí cho từng người vay để lợi nhuận kỳ vọng của ngân hàng bạn bằng 0 không? Giả sử rằng bạn không nhận được x từ những người đi vay không trả được nợ. Cũng lưu ý x không phải là lãi suất mà là tổng số tiền bạn thêm vào. Chúng ta có thể tham khảo x chia cho kích thước (x/180000) là lãi suất .

22. Viết lại mô hình mẫu từ 20 và chạy mô phỏng Monte Carlo để có ý tưởng về việc phân phối lợi nhuận khi bạn tính lãi suất.

23. Tuy nhiên, chúng ta không thực sự cần mô phỏng Monte Carlo. Sử dụng những gì chúng ta đã học được, CLT cho chúng ta biết rằng vì số tiền thua lỗ của chúng ta là tổng của các lần rút độc lập nên mức phân phối của nó gần như bình thường. Giá trị kỳ vọng và sai số chuẩn của lợi nhuận S? Viết chúng dưới dạng hàm của xác suất bị tịch thu tài sản thế chấp p, số lượng khoản vay n, tổn thất trên mỗi lần tịch thu tài sản thế chấp l và số tiền bạn tính cho mỗi người vay x.

24. Nếu bạn đặt x để đảm bảo ngân hàng của bạn hòa vốn (lợi nhuận kỳ vọng là 0), xác suất ngân hàng của bạn thua lỗ là bao nhiêu?

25. Giả sử nếu ngân hàng của bạn có lợi nhuận âm thì ngân hàng đó phải đóng cửa. Vì vậy bạn cần phải tăng x để giảm thiểu rủi ro này. Tuy nhiên, nếu bạn đặt lãi suất quá cao, khách hàng của bạn sẽ tìm đến ngân hàng khác. Vì vậy, giả sử chúng ta muốn khả năng mất tiền của mình là 1 trên 100, thì tỷ lệ là bao nhiêu? x số lượng cần phải có ngay bây giờ? Gợi ý: Chúng tôi muốn
 Pr(S < 0) = 0.01. Lưu ý rằng bạn có thể cộng các hằng số trừ cho cả hai vế của bất đẳng thức và xác suất không thay đổi:
Pr(S < 0) = Pr(S + k < 0 + k), Tương tự, với phép chia hằng số dương: Pr(S + k < 0 + k) = Pr((S + k)/m < k/m). Sử dụng thực tế này và CLT để biến đổi vế trái của bất đẳng thức thành Pr(S < 0) thành một tiêu chuẩn bình thường.

26. Lãi suất của chúng tôi bây giờ tăng lên. Nhưng đây vẫn là mức lãi suất rất cạnh tranh. Cho x bạn thu được trong năm 25, lợi nhuận kỳ vọng trên mỗi khoản vay là bao nhiêu và tổng lợi nhuận kỳ vọng là bao nhiêu?

27. Chạy mô phỏng Monte Carlo để kiểm tra kỹ phép tính gần đúng lý thuyết được sử dụng trong 25 và 26.

28. Một nhân viên của bạn chỉ ra rằng vì ngân hàng đang kiếm được lợi nhuận trên mỗi khoản vay nên ngân hàng nên cho vay nhiều hơn! Tại sao chỉ n? Bạn giải thích rằng việc tìm kiếm những n khách hàng thật khó khăn. Bạn cần một nhóm có thể dự đoán được và giữ cho khả năng vỡ nợ ở mức thấp. Sau đó, ông chỉ ra rằng ngay cả khi xác suất vỡ nợ cao hơn, miễn là giá trị kỳ vọng của chúng tôi là dương, bạn có thể giảm thiểu khả năng thua lỗ bằng cách tăng n và dựa vào luật số lớn. Giả sử xác suất vỡ nợ cao gấp đôi, hay 4%, và bạn đặt lãi suất ở mức 5%, hoặc x = 9.000 USD, lợi nhuận kỳ vọng của bạn trên mỗi khoản vay là bao nhiêu?

29. Chúng ta phải tăng bao nhiêu n bằng cách đảm bảo xác suất mất tiền vẫn nhỏ hơn 0,01?

30. Xác nhận kết quả ở phần 29 bằng mô phỏng Monte Carlo.

31. Theo phương trình này, việc cho vay nhiều hơn sẽ làm tăng lợi nhuận dự kiến ​​của bạn và giảm nguy cơ mất tiền! Cho vay nhiều hơn có vẻ như là một điều không cần phải cân nhắc. Kết quả là đồng nghiệp của bạn quyết định rời khỏi ngân hàng của bạn và thành lập công ty thế chấp rủi ro cao của riêng mình. Vài tháng sau, ngân hàng của đồng nghiệp bạn phá sản. Một cuốn sách được viết và cuối cùng các bộ phim The Big Short và Margin Call được thực hiện liên quan đến sai lầm mà bạn của bạn và nhiều người khác đã mắc phải. Chuyện gì đã xảy ra thế?

Kế hoạch của đồng nghiệp của bạn chủ yếu dựa trên công thức toán học này SE(mean(X)) = σ/sqrt(n). Bằng việc tạo ra
lớn, chúng ta giảm thiểu sai số chuẩn của lợi nhuận trên mỗi khoản vay. Tuy nhiên, để quy định này được giữ vững, Xs phải rút ra độc lập: việc vỡ nợ của một người phải độc lập với việc vỡ nợ của người khác.

Để xây dựng một mô phỏng thực tế hơn mô phỏng ban đầu mà đồng nghiệp của bạn đã thực hiện, giả sử có một sự kiện toàn cầu ảnh hưởng đến mọi người có khoản thế chấp rủi ro cao và làm thay đổi xác suất của họ. Chúng ta sẽ giả định rằng với cơ hội 50-50, tất cả các xác suất sẽ tăng hoặc giảm nhẹ ở khoảng từ 0,03 đến 0,05. Nhưng nó xảy ra với tất cả mọi người cùng một lúc chứ không phải chỉ một người. Những lần rút thăm này không còn độc lập nên phương trình của chúng tôi không được áp dụng. Viết mô phỏng Monte Carlo cho tổng lợi nhuận của bạn với mô hình này.

32. Sử dụng kết quả mô phỏng để báo cáo lợi nhuận dự kiến, xác suất thua lỗ và xác suất thua lỗ hơn 10.000.000 USD. Nghiên cứu việc phân phối lợi nhuận và thảo luận việc đưa ra giả định sai sẽ dẫn đến kết quả thảm khốc như thế nào,