...
Chúng tôi muốn dự đoán tỷ lệ hạt màu xanh trong bình. Hãy gọi số lượng p này, sau đó cho chúng ta biết tỷ lệ của các hạt màu đỏ 1 - p, và sự lây lan p - (1 - p), điều này đơn giản hóa thành 2p - 1.
...
Chúng ta có thể làm điều này với 25 quan sát trên không? Nó chắc chắn là thông tin. Ví dụ, cho rằng chúng ta nhìn thấy 13 hạt màu đỏ và 12 hạt màu xanh, điều đó khó có thể xảy ra. p > 9 hoặc p < 1. Nhưng chúng ta đã sẵn sàng dự đoán một cách chắc chắn rằng có nhiều hạt màu đỏ hơn hạt màu xanh trong lọ chưa?
...
0.48 ít nhất phải liên quan đến tỷ lệ thực tế p. Nhưng liệu chúng ta có đơn giản dự đoán p là 0,48? Đầu tiên, hãy nhớ rằng tỷ lệ mẫu là một biến ngẫu nhiên. Nếu chúng ta chạy lệnh take_poll(25) bốn lần, mỗi lần chúng ta sẽ nhận được một câu trả lời khác nhau vì tỷ lệ mẫu là một biến ngẫu nhiên.
...
Việc tiến hành một cuộc thăm dò ý kiến ​​đang được mô hình hóa như lấy một mẫu ngẫu nhiên từ một chiếc bình. Chúng tôi đang đề xuất sử dụng tỷ lệ hạt màu xanh lam trong mẫu của chúng tôi để ước tính tham số p. Sau khi có được ước tính này, chúng tôi có thể dễ dàng báo cáo ước tính về mức chênh lệch 2p - 1, nhưng để đơn giản chúng tôi sẽ minh họa các khái niệm để ước lượng p. Chúng ta sẽ sử dụng kiến ​​thức về xác suất để bảo vệ việc sử dụng tỷ lệ mẫu và định lượng mức độ chênh lệch giữa chúng ta nghĩ với tỷ lệ tổng thể p. 

Chúng tôi bắt đầu bằng cách xác định biến ngẫu nhiên X BẰNG: X = 1 nếu chúng ta chọn ngẫu nhiên một hạt màu xanh và X = 0 nếu nó có màu đỏ. Điều này ngụ ý rằng tổng thể là một danh sách gồm các số 0 và 1. Nếu chúng ta lấy mẫu N hạt, sau đó là giá trị trung bình của các lần rút X(1),..,X(N) tương đương với tỷ lệ hạt màu xanh trong mẫu của chúng tôi. Điều này là do việc thêm Xs tương đương với việc đếm các hạt màu xanh và chia số này cho tổng số N tương đương với việc tính toán một tỷ lệ. Chúng tôi sử dụng ký hiệu X̅ để đại diện cho mức trung bình này. Nói chung, trong sách giáo khoa thống kê, một thanh phía trên ký hiệu có nghĩa là mức trung bình. Lý thuyết chúng ta vừa học về tổng số lần rút trở nên hữu ích vì trung bình là tổng số lần rút nhân với hằng số 1/N:
$$\bar{X} = \frac{1}{N} \sum_{i=1}^N X_i$$
Để đơn giản, hãy giả sử rằng các lần rút là độc lập: sau khi chúng tôi nhìn thấy từng hạt được lấy mẫu, chúng tôi sẽ đưa nó trở lại bình. Trong trường hợp này, chúng ta biết gì về việc phân bổ tổng số tiền rút? Đầu tiên, chúng ta biết rằng giá trị kỳ vọng của tổng số lần rút là N lần giá trị trung bình của các giá trị trong bình. Chúng ta biết rằng trung bình của số 0 và số 1 trong bình phải là p, tỷ lệ của hạt màu xanh.

Ở đây chúng ta nhận thấy một sự khác biệt quan trọng với những gì chúng ta đã làm trong chương Xác suất: chúng ta không biết trong bình có gì. Chúng ta biết có những hạt xanh và đỏ, nhưng chúng ta không biết mỗi hạt có bao nhiêu hạt. Đây là điều chúng tôi muốn tìm hiểu: chúng tôi đang cố gắng ước tính p.

6.2.2 Thông số
Giống như việc chúng ta sử dụng các biến để xác định các ẩn số trong hệ phương trình, trong suy luận thống kê, chúng ta xác định các tham số để xác định các phần chưa biết trong mô hình của mình. Trong mô hình chiếc bình mà chúng tôi đang sử dụng để bắt chước một cuộc thăm dò ý kiến, chúng tôi không biết tỷ lệ các hạt màu xanh lam trong chiếc bình. Chúng tôi xác định các tham số p để biểu thị đại lượng này. p là mức trung bình của chiếc bình vì nếu lấy trung bình của số 1 (màu xanh) và số 0 (màu đỏ), chúng ta sẽ có được tỷ lệ các hạt màu xanh. Vì mục tiêu chính của chúng tôi là tìm ra những gì p, chúng ta sẽ ước tính tham số này .
...
Trước khi tiếp tục, chúng ta hãy làm rõ một vấn đề quan trọng liên quan đến vấn đề thực tế trong việc dự báo cuộc bầu cử. Nếu một cuộc thăm dò được tiến hành bốn tháng trước cuộc bầu cử, nó sẽ ước tính tỷ lệ p vào thời điểm đó chứ không phải cho ngày bầu cử. Các p cho đêm bầu cử có thể khác vì quan điểm của mọi người dao động theo thời gian. Các cuộc thăm dò được thực hiện vào đêm trước cuộc bầu cử có xu hướng chính xác nhất vì các ý kiến ​​không thay đổi nhiều trong một ngày. Tuy nhiên, các nhà dự báo cố gắng xây dựng các công cụ mô hình hóa mức độ thay đổi của các ý kiến ​​theo thời gian và cố gắng dự đoán kết quả của đêm bầu cử có tính đến thực tế là các ý kiến ​​luôn biến động. Chúng tôi sẽ mô tả một số phương pháp để thực hiện việc này trong phần sau.

6.4 Đặc tính ước tính của chúng tôi: giá trị kỳ vọng và sai số chuẩn
Để hiểu ước tính của chúng tôi tốt đến mức nào, chúng tôi sẽ mô tả các đặc tính thống kê của biến ngẫu nhiên được xác định ở trên: tỷ lệ mẫu X̅. Nhớ lấy X̅ là tổng của các lần rút độc lập nên các quy tắc chúng tôi đề cập trong chương xác suất sẽ được áp dụng.

Sử dụng những gì chúng ta đã học, giá trị kỳ vọng của tổng NX̅ là
Nx mức trung bình của bình,p. Vì vậy chia cho hằng số không ngẫu nhiên N cho chúng ta biết giá trị kỳ vọng của giá trị trung bình
X̅ là p. Chúng ta có thể viết nó bằng ký hiệu toán học của chúng ta:
$$
\mbox{E}(\bar{X}) = p
$$
Chúng ta cũng có thể sử dụng những gì đã học để tìm ra sai số chuẩn: sai số chuẩn của tổng là sqrt(N)x độ lệch chuẩn của bình. Chúng ta có thể tính được sai số chuẩn của chiếc bình không? Chúng tôi đã học được một công thức cho chúng tôi biết rằng đó là (1 - 0)sqrt(p(1-p))= sqrt(p(1-p)). Bởi vì chúng ta đang chia tổng cho N, chúng ta đi đến công thức sau đây cho sai số chuẩn của giá trị trung bình:
$$
\mbox{SE}(\bar{X}) = \sqrt{p(1-p)/N}
$$
Kết quả này cho thấy sức mạnh của các cuộc thăm dò ý kiến. Giá trị kỳ vọng của tỷ lệ mẫu X̅ là tham số quan tâm p và chúng ta có thể làm cho sai số chuẩn nhỏ như chúng ta muốn bằng cách tăng N. Luật số lớn cho chúng ta biết rằng với một cuộc thăm dò đủ lớn, ước tính của chúng ta sẽ hội tụ về p.
...
Một vấn đề là chúng ta không biết p, vì vậy chúng tôi không thể tính được sai số chuẩn. Tuy nhiên, nhằm mục đích minh họa, hãy giả sử rằng p = 0.51 và vẽ đồ thị sai số chuẩn so với cỡ mẫu N:
...
```
Từ cốt truyện, chúng ta thấy rằng chúng ta sẽ cần một cuộc thăm dò ý kiến ​​của hơn 10.000 người để có được sai số chuẩn thấp đến mức đó. Chúng tôi hiếm khi thấy các cuộc thăm dò quy mô này một phần do chi phí. Từ bảng Chính trị rõ ràng thực sự, chúng tôi biết rằng quy mô mẫu trong các cuộc thăm dò dư luận dao động từ 500-3.500 người. Với cỡ mẫu là 1.000 và p = 0.51, lỗi tiêu chuẩn là:
```{r}
sqrt(p*(1 - p))/sqrt(1000)
```
hoặc 1,5 điểm phần trăm. Vì vậy ngay cả với các cuộc thăm dò lớn, đối với các cuộc bầu cử sát sao X̅, có thể khiến chúng ta lạc lối nếu chúng ta không nhận ra đó là một biến ngẫu nhiên. Tuy nhiên, chúng ta thực sự có thể nói nhiều hơn về mức độ chúng ta đạt được p và chúng tôi làm điều đó trong Phần Chương 7 .

7   Định lý giới hạn trung tâm
CLT cho chúng ta biết rằng hàm phân phối của tổng số lần rút gần như bình thường. Chúng ta cũng đã biết rằng việc chia một biến ngẫu nhiên có phân phối chuẩn cho một hằng số cũng là một biến có phân phối chuẩn. Điều này hàm ý rằng việc phân phối X̅ là gần như bình thường.

Tóm lại, chúng ta có điều đó X̅ có phân phối xấp xỉ chuẩn với giá trị kỳ vọnvọng p và sai số chuẩn sqrt(p(1-p)/N).

Bây giờ điều này giúp chúng ta như thế nào? Giả sử chúng ta muốn biết xác suất để chúng ta nằm trong phạm vi 1% tính từ pp. Về cơ bản chúng tôi đang hỏi cái gì là
$$
...
$$
Chúng ta có thể trả lời câu hỏi này không? Chúng ta có thể sử dụng thủ thuật toán học đã học ở chương trước. Trừ giá trị kỳ vọng và chia cho sai số chuẩn để được biến ngẫu nhiên chuẩn hóa chuẩn, gọi nó là Z, bên trái. Từ p là giá trị kỳ vọng và SE(X̅) = sqrt(p(1-p)/N) là lỗi tiêu chuẩn chúng tôi nhận được:
$$
\mbox{Pr}\left(Z \leq \frac{ \,.01} {\mbox{SE}(\bar{X})} \right) -
\mbox{Pr}\left(Z \leq - \frac{ \,.01} {\mbox{SE}(\bar{X})} \right) 
$$
Một vấn đề chúng tôi gặp phải là vì chúng tôi không biết p, chúng tôi không biết SE(X̅). Nhưng hóa ra CLT vẫn hoạt động nếu chúng ta ước tính sai số chuẩn bằng cách sử dụng X̅ thay cho p. Chúng tôi nói rằng chúng tôi đưa vào ước tính. Do đó, ước tính của chúng tôi về sai số chuẩn là:
$$
\hat{\mbox{SE}}(\bar{X})=\sqrt{\bar{X}(1-\bar{X})/N}
$$
Trong sách giáo khoa thống kê, chúng ta sử dụng một chiếc mũ nhỏ để biểu thị các ước tính. Ước tính có thể được xây dựng bằng cách sử dụng dữ liệu được quan sát và N.

Bây giờ chúng ta tiếp tục tính toán nhưng chia cho SE(X̅) = sqrt(X̅(1-X̅)/N) thay vì. Trong mẫu đầu tiên của chúng tôi, chúng tôi có 12 màu xanh và 13 màu đỏ X̅ = 0.48 và ước tính sai số chuẩn của chúng tôi là:
```{r}
x_hat <- 0.48
se <- sqrt(x_hat*(1-x_hat)/25)
se
```
Và bây giờ chúng ta có thể trả lời câu hỏi về xác suất gần với pp. Câu trả lời là:
```{r}
pnorm(0.01/se) - pnorm(-0.01/se)
```
Vì vậy, có rất ít khả năng chúng ta sẽ thân thiết. Một cuộc thăm dò ý kiến ​​chỉ N = 25 mọi người không thực sự hữu ích lắm, ít nhất là không cho một cuộc bầu cử sát nút.
...
1.96*se
```
Tại sao chúng ta nhân với 1,96? Bởi vì nếu bạn hỏi xác suất để chúng ta nằm trong phạm vi 1,96 sai số chuẩn là bao nhiêu? p, chúng tôi nhận được:
$$
...
```
Do đó, có xác suất 95% rằng X̅ sẽ ở trong 1.96 x SE(X̅), trong trường hợp của chúng tôi trong khoảng 0,2,p. Lưu ý rằng 95% là một lựa chọn tùy ý và đôi khi các tỷ lệ phần trăm khác cũng được sử dụng, nhưng đó là giá trị được sử dụng phổ biến nhất để xác định biên độ sai số. Chúng tôi thường làm tròn 1,96 lên 2 để đơn giản cho việc trình bày.

Tóm lại, CLT cho chúng ta biết rằng cuộc thăm dò của chúng tôi dựa trên cỡ mẫu là 25 không hữu ích lắm. Chúng tôi thực sự không học được nhiều khi biên độ sai sót quá lớn. Tất cả những gì chúng tôi thực sự có thể nói là số phiếu phổ thông sẽ không giành được với tỷ lệ chênh lệch lớn. Đây là lý do tại sao những người thăm dò ý kiến ​​có xu hướng sử dụng cỡ mẫu lớn hơn.

Từ bảng trên, chúng ta thấy rằng cỡ mẫu điển hình nằm trong khoảng từ 700 đến 3500. Để xem điều này mang lại cho chúng ta kết quả thực tế hơn nhiều như thế nào, hãy lưu ý rằng nếu chúng ta thu được một X̅ = 0,48 với cỡ mẫu là 2.000, sai số chuẩn của chúng tôi SE(X̅) sẽ là 0,0111714. Vì vậy, kết quả của chúng tôi là ước tính 48% với sai số 2%. Trong trường hợp này, kết quả mang lại nhiều thông tin hơn và khiến chúng ta nghĩ rằng có nhiều quả bóng màu đỏ hơn quả bóng màu xanh. Tuy nhiên, hãy nhớ rằng đây chỉ là giả thuyết. Chúng tôi đã không thực hiện cuộc thăm dò 2.000 người vì chúng tôi không muốn làm hỏng cuộc thi.
...
```
Để xem xét lại, lý thuyết cho chúng ta biết rằng X̅ có phân phối xấp xỉ chuẩn, có giá trị kỳ vọng p = 0,45 và sai số chuẩn sqrt(p(1-p)/N) = 0,0157321. Mô phỏng xác nhận điều này:
```{r}
mean(x_hat)
sd(x_hat)
```
...
```
Tất nhiên, trong đời thực, chúng ta sẽ không bao giờ có thể tiến hành một thí nghiệm như vậy vì chúng ta không biết pp. Nhưng chúng ta có thể chạy nó với nhiều giá trị khác nhau của p Và N và thấy rằng lý thuyết này thực sự hoạt động tốt với hầu hết các giá trị. Bạn có thể dễ dàng thực hiện việc này bằng cách chạy lại mã ở trên sau khi thay đổi p và N.

7.2 Sự lây lan
Cạnh tranh là để dự đoán mức độ lây lan chứ không phải tỷ lệ p. Tuy nhiên, vì chúng ta giả định chỉ có hai bên nên chúng ta biết rằng mức chênh lệch là μ = p - (1 - p) = 2p - 1. Kết quả là mọi thứ chúng tôi đã làm có thể dễ dàng được điều chỉnh cho phù hợp với ước tính μ. Một khi chúng tôi có ước tính của mình X̅ Và SE(X̅), chúng tôi ước tính mức chênh lệch với 2X̅ - 1 và vì chúng ta đang nhân với 2 nên sai số chuẩn là 2SE(X̅). Lưu ý rằng việc trừ 1 không thêm bất kỳ sự thay đổi nào nên nó không ảnh hưởng đến sai số chuẩn.

Đối với mẫu 25 mục ở trên, ước tính của chúng tôi có p.48sai số .20và ước tính của chúng tôi về mức chênh lệch 0.04có sai số .40. Một lần nữa, đây không phải là cỡ mẫu hữu ích. Tuy nhiên, vấn đề là khi chúng ta có ước tính và sai số chuẩn cho p, chúng tôi có nó để lây lan μ.

7.3 Xu hướng: tại sao không tổ chức một cuộc thăm dò thật lớn?
Đối với các giá trị thực tế của p, chẳng hạn từ 0,35 đến 0,65, nếu chúng ta thực hiện một cuộc thăm dò rất lớn với 100.000 người, lý thuyết cho chúng ta biết rằng chúng ta sẽ dự đoán cuộc bầu cử một cách hoàn hảo vì sai số lớn nhất có thể xảy ra là khoảng 0,3%:
...
Trong cuộc thi trước đây của chúng tôi, bạn được yêu cầu nghỉ giải lao. Nếu khoảng thời gian bạn gửi bao gồm p, bạn sẽ nhận lại được một nửa số tiền đã chi cho "cuộc thăm dò ý kiến" của mình và chuyển sang giai đoạn tiếp theo của cuộc thi. Một cách để vượt qua vòng thứ hai là báo cáo một khoảng thời gian rất lớn. Ví dụ, khoảng [0,1] được đảm bảo bao gồm p. Tuy nhiên, với khoảng cách lớn như vậy, chúng ta không có cơ hội giành chiến thắng trong cuộc thi. Tương tự, nếu bạn là người dự báo bầu cử và dự đoán tỷ lệ chênh lệch sẽ nằm trong khoảng từ -100% đến 100%, bạn sẽ bị chế giễu vì đã nêu ra điều hiển nhiên. Ngay cả một khoảng thời gian nhỏ hơn, chẳng hạn như nói rằng mức chênh lệch sẽ nằm trong khoảng từ -10 đến 10%, sẽ không được coi là nghiêm trọng.
...
Chúng ta có thể sử dụng lý thuyết thống kê đã học để tính xác suất của bất kỳ khoảng thời gian nào, bao gồm cả p. Nếu chúng ta được yêu cầu tạo một khoảng với 95% cơ hội bao gồm p, chúng ta cũng có thể làm được điều đó. Chúng được gọi là khoảng tin cậy 95%.
...
Để xác định xác suất mà khoảng bao gồm p, chúng ta cần tính toán:
$$
...
$$
Số hạng ở giữa là một biến ngẫu nhiên xấp xỉ bình thường với giá trị kỳ vọng là 0 và sai số chuẩn 1, mà chúng ta đã ký hiệu là Z, vì vậy chúng tôi có:
$$
...
```
là 0.995 - 0.005 = 0.99.
Chúng ta có thể sử dụng phương pháp này cho bất kỳ xác suất nào, không chỉ 0,95 và 0,99. Trong sách giáo khoa thống kê, chúng thường được viết cho bất kỳ xác suất nào như 1 - a. Sau đó chúng ta có thể có được z cho phương trình trên lưu ý sử dụng z = qnorm(1 - alpha / 2) vì 1 - a/2 - a/2 = 1 - a.
Vì vậy, ví dụ, đối với a = 0.05, 1 - a/2 = 0.975 và chúng tôi nhận được 1,96 mà chúng tôi đang sử dụng:
```{r}
qnorm(0.975)
#> [1] 1.96
```
8.1 Mô phỏng Monte Carlo
Chúng ta có thể chạy mô phỏng Monte Carlo để xác nhận rằng, trên thực tế, khoảng tin cậy 95% bao gồm p 95% thời gian.
```{r}
N <- 1000
B <- 10000
inside <- replicate(B, {
...
Khi sử dụng lý thuyết mà chúng tôi đã mô tả ở trên, điều quan trọng cần nhớ là các khoảng thời gian là ngẫu nhiên chứ không phải p. Trong biểu đồ trên, chúng ta có thể thấy các khoảng ngẫu nhiên di chuyển xung quanh và p, được biểu thị bằng đường thẳng đứng, giữ nguyên vị trí. Tỷ lệ màu xanh trong bình p không phải. Vì vậy, 95% liên quan đến xác suất khoảng ngẫu nhiên này rơi vào p. Nói p có 95% khả năng nằm giữa cái này và cái kia về mặt kỹ thuật là một tuyên bố không chính xác vì p không phải là ngẫu nhiên. 
:::
...
9,1 giá trị p
Giả sử chúng ta lấy một mẫu ngẫu nhiên của N = 100 và chúng tôi quan sát 52 những hạt màu xanh, mang đến cho chúng ta X̅=0.52. Điều này dường như chỉ ra sự tồn tại của nhiều hạt màu xanh hơn hạt màu đỏ vì 0,52 lớn hơn 0,5. Tuy nhiên, chúng tôi biết có cơ hội tham gia vào quá trình này và chúng tôi có thể đạt được điểm 52 ngay cả khi thực tế p = 0.5. Chúng ta gọi giả định đó là p =0.5 một giả thuyết vô hiệu . Giả thuyết không là giả thuyết của người hoài nghi.

Chúng tôi đã quan sát thấy một biến ngẫu nhiên X̅ = 0.52 và giá trị p là câu trả lời cho câu hỏi: khả năng nhìn thấy giá trị lớn như thế này là bao nhiêu khi giả thuyết khống là đúng? Nếu giá trị p đủ nhỏ, chúng tôi bác bỏ giả thuyết không và nói rằng kết quả có ý nghĩa thống kê .
...
giả sử p = 0.5. Theo giả thuyết không, chúng ta biết rằng:
$$
...
Chúng ta có thể chứng minh một cách toán học rằng nếu (1 - a)x100 Khoảng tin cậy % không chứa giá trị giả thuyết không, giả thuyết không bị bác bỏ với giá trị p nhỏ hơn hoặc nhỏ hơn a. Vì vậy, ý nghĩa thống kê có thể được xác định từ khoảng tin cậy. Tuy nhiên, không giống như khoảng tin cậy, giá trị p không đưa ra ước tính về mức độ ảnh hưởng. Vì lý do này, chúng tôi khuyên bạn nên tránh sử dụng giá trị p bất cứ khi nào bạn có thể tính khoảng tin cậy.
...
Một vấn đề với kết quả cuộc thăm dò ý kiến ​​của chúng tôi là với kích thước mẫu và giá trị của p, chúng ta sẽ phải hy sinh xác suất của một cuộc gọi sai để tạo ra một khoảng không bao gồm 0.
...
#> [1] 11269
```
những người tham gia. Về cơ bản, chúng tôi xây dựng một ước tính về mức chênh lệch, hãy gọi nó là μ, với giá trị trung bình có trọng số như sau:
```{r}
mu_hat <- polls |> 
  summarize(avg = sum(estimate*sample_size) / sum(sample_size)) |> 
  pull(avg)
```
Một khi chúng ta có ước tính về μ, chúng ta có thể xây dựng ước tính về tỷ lệ bỏ phiếu cho Obama, sau đó chúng ta có thể sử dụng ước tính này để ước tính sai số chuẩn. Khi thực hiện việc này, chúng tôi thấy rằng biên độ sai số của chúng tôi là 0,0184545.
...
```
Trong ví dụ này, chúng ta sẽ giả định rằng chỉ có hai bên và gọi p tỷ lệ bỏ phiếu cho Clinton và 1 - p tỷ lệ bỏ phiếu cho Trump. Chúng tôi quan tâm đến sự lan truyền 2p - 1. Hãy gọi sự lây lan μ (vì sự khác biệt).

Chúng tôi có 49 ước tính về mức chênh lệch. Lý thuyết mà chúng ta học được từ các mô hình lấy mẫu cho chúng ta biết rằng những ước tính này là một biến ngẫu nhiên có phân bố xác suất gần như bình thường. Giá trị kỳ vọng là đêm bầu cử lan rộng μ và sai số chuẩn là
2sqrt(p(1-p)/N). Giả sử mô hình chiếc bình mà chúng tôi mô tả trước đó là một mô hình tốt, chúng tôi có thể sử dụng thông tin này để xây dựng khoảng tin cậy dựa trên dữ liệu tổng hợp. Mức chênh lệch ước tính là:
```{r}
mu_hat <- polls |> 
...
```
Mặc dù chúng tôi không còn sử dụng mô hình có hạt màu đỏ (Cộng hòa) và xanh lam (Đảng Dân chủ) trong bình đựng tro cốt, mô hình mới của chúng tôi cũng có thể được coi là chế độ bình đựng tro cốt nhưng chứa kết quả thăm dò ý kiến ​​từ tất cả những người thăm dò ý kiến ​​có thể có và hãy nghĩ đến $N= của chúng tôi điểm dữ liệu $15 X1,...,XN a như một mẫu ngẫu nhiên từ chiếc bình này. Để phát triển một mô hình hữu ích, chúng tôi giả định rằng giá trị kỳ vọng của chiếc bình của chúng tôi là mức chênh lệch thực tế μ = 2p - 1, ngụ ý rằng trung bình mẫu có giá trị kỳ vọng μ.

Bây giờ, vì thay vì 0 và 1, chiếc bình của chúng ta chứa các số liên tục nên độ lệch chuẩn của chiếc bình không còn nữa sqrt(p(1 - p). Thay vì mức độ biến thiên trong việc lấy mẫu của cử tri, lỗi tiêu chuẩn hiện bao gồm mức độ biến thiên giữa người thăm dò ý kiến. Chiếc bình mới của chúng tôi cũng bao gồm khả năng biến thiên lấy mẫu từ cuộc bỏ phiếu. Bất chấp điều đó, độ lệch chuẩn này hiện là một tham số chưa xác định. Trong sách giáo khoa thống kê, ký hiệu Hy Lạp σ được sử dụng để đại diện cho tham số này.

Vì vậy, mô hình thống kê mới của chúng tôi là X1,...,XN là một mẫu ngẫu nhiên với kỳ vọng μ và độ lệch chuẩn σ. Hiện tại, việc phân phối vẫn chưa được xác định. Nhưng chúng tôi xem xét N đủ lớn để giả định rằng trung bình mẫu $\bar{X} = \sum_{i=1}^N X_i$ tuân theo phân phối chuẩn với giá trị kỳ vọng μ và sai số chuẩn σ/sqrt(N). Chúng tôi viết
$$
\bar{X} \sim \mbox{N}(\mu, \sigma / \sqrt{N})
$$
Đây là ~ ký hiệu cho chúng ta biết biến ngẫu nhiên ở bên trái ký hiệu tuân theo phân phối ở bên phải. Chúng tôi sử dụng ký hiệu
N(a,b) để biểu diễn phân phối chuẩn với giá trị trung bình a và độ lệch chuẩn b.

Mô hình trung bình mẫu này sẽ được sử dụng lại ở chương tiếp theo.

10.2.1 Ước tính độ lệch chuẩn
Mô hình chúng tôi đã chỉ định có hai tham số chưa xác định: giá trị mong đợi μ và độ lệch chuẩn σ. Chúng ta biết rằng trung bình mẫu X̅
sẽ là ước tính của chúng tôi về μ. Nhưng còn σ?

Nhiệm vụ của chúng ta là ước tính μ. Bởi vì chúng tôi lập mô hình các giá trị quan sát được X1,...,XN dưới dạng mẫu ngẫu nhiên từ bình đựng mẫu, với cỡ mẫu đủ lớn N, phân bố xác suất của trung bình mẫu X̅ xấp xỉ bình thường với giá trị kỳ vọng μ và sai số chuẩn σ/sqrt(N). Nếu chúng ta sẵn lòng xem xét N = 15 đủ lớn, chúng ta có thể sử dụng điều này để xây dựng khoảng tin cậy.

Lý thuyết cho chúng ta biết rằng chúng ta có thể ước tính mô hình chiếc bình σ với độ lệch chuẩn mẫu được xác định là
$$
s = \sqrt{ \frac{1}{N-1} \sum_{i=1}^N (X_i - \bar{X})^2 }
$$
Lưu ý rằng không giống như định nghĩa về độ lệch chuẩn của tổng thể, bây giờ chúng ta chia cho N - 1.  Điều này làm cho s một ước tính tốt hơn về σ.Có một lời giải thích toán học cho điều này, được giải thích trong hầu hết các sách giáo khoa thống kê, nhưng chúng tôi không đề cập đến nó ở đây.
...
10.2.3 Phân phối t
Ở trên chúng tôi đã sử dụng CLT với cỡ mẫu là 15. Bởi vì chúng tôi đang ước tính tham số thứ hai σ, sự thay đổi hơn nữa được đưa vào khoảng tin cậy của chúng tôi dẫn đến khoảng thời gian quá nhỏ. Đối với cỡ mẫu rất lớn, độ biến thiên bổ sung này là không đáng kể, nhưng nói chung, đối với các giá trị nhỏ hơn 30, chúng ta cần thận trọng khi sử dụng CLT. Tuy nhiên, nếu dữ liệu trong bình được biết là tuân theo phân phối chuẩn thì chúng ta thực sự có lý thuyết toán học cho chúng ta biết chúng ta cần tạo các khoảng lớn hơn bao nhiêu để tính toán cho ước tính của σ. Sử dụng lý thuyết này, chúng ta có thể xây dựng khoảng tin cậy cho bất kỳ N. Nhưng một lần nữa, điều này chỉ có tác dụng nếu dữ liệu trong bình được biết là tuân theo phân phối chuẩn . Vì vậy, đối với dữ liệu 0, 1 của mô hình chiếc bình trước đây của chúng tôi, lý thuyết này chắc chắn không áp dụng được.

Thống kê về khoảng tin cậy cho μ dựa trên là
$$
Z = \frac{\bar{X} - \mu}{\sigma/\sqrt{N}}
$$
CLT cho chúng ta biết rằng Z có phân phối xấp xỉ chuẩn với giá trị kỳ vọng là 0 và sai số chuẩn 1. Nhưng trong thực tế, chúng ta không biết σ vì vậy chúng tôi sử dụng:
$$
t = \frac{\bar{X} - \mu}{s/\sqrt{N}}
$$ 
Điều này được đề cập đến một thống kê t . Bằng cách thay thế σ với s chúng tôi giới thiệu một số biến thể. Lý thuyết cho chúng ta biết rằng t tuân theo phân phối t của sinh viên với N - 1 bậc tự do . Bậc tự do là một tham số kiểm soát sự biến thiên thông qua các đuôi béo hơn:
...
```
Nếu chúng ta sẵn sàng giả định rằng dữ liệu về hiệu ứng người thăm dò ý kiến ​​có phân phối chuẩn, dựa trên dữ liệu mẫu X1,...,XN,
```{r}
one_poll_per_pollster |>
  ggplot(aes(sample = spread)) + stat_qq()
```
sau đó t tuân theo phân phối t với N - 1 bậc tự do. Vì vậy có lẽ khoảng tin cậy tốt hơn cho μ là:
```{r}
...
```
Nhưng điều này có ý nghĩa gì trong bối cảnh lý thuyết mà chúng ta đã đề cập trong đó những tỷ lệ phần trăm này được coi là cố định. Hơn nữa, các nhà dự báo bầu cử còn đưa ra những tuyên bố mang tính xác suất như “Obama có 90% cơ hội thắng cử”. Lưu ý rằng trong bối cảnh của mô hình chiếc bình, điều này tương đương với việc phát biểu rằng xác suất p > 0.5 là 90%. Tuy nhiên, mẫu bình p là một tham số cố định và không có ý nghĩa gì khi nói về xác suất. Với thống kê Bayesian, chúng tôi lập mô hình p là biến ngẫu nhiên và do đó tuyên bố như “90% cơ hội chiến thắng” phù hợp với phương pháp toán học. Các nhà dự báo cũng sử dụng các mô hình để mô tả sự biến đổi ở các cấp độ khác nhau. Ví dụ: độ biến thiên của việc lấy mẫu, độ biến thiên của người thăm dò ý kiến, độ biến thiên hàng ngày và độ biến thiên của cuộc bầu cử. Một trong những cách tiếp cận thành công nhất được sử dụng cho việc này là các mô hình phân cấp, có thể được giải thích trong bối cảnh thống kê Bayes.
...
$$
với + có nghĩa là một bài kiểm tra tích cực và D đại diện cho việc bạn thực sự mắc bệnh (1) hay không (0).

Giả sử chúng ta chọn một người ngẫu nhiên và họ có kết quả dương tính. Xác suất họ mắc bệnh là bao nhiêu? Chúng tôi viết điều này như Prob(D = 1| +) ?Tỷ lệ xơ nang là 1 trên 3.900, điều này ngụ ý rằng Prob(D = 1) = 0.00025. Để trả lời câu hỏi này, chúng ta sẽ sử dụng định lý Bayes, định lý này nói chung cho chúng ta biết rằng:
$$
...
Trong chương trước, chúng tôi ước tính và biên độ sai số về sự khác biệt về số phiếu phổ thông giữa Hillary Clinton và Donald Trump, mà chúng tôi ký hiệu bằng μ. Ước tính nằm trong khoảng từ 2 đến 3 phần trăm và khoảng tin cậy không bao gồm 0. Một nhà dự báo sẽ sử dụng điều này để dự đoán Hillary Clinton sẽ giành được số phiếu phổ thông. Nhưng để đưa ra tuyên bố xác suất về việc thắng cử, chúng ta cần sử dụng phương pháp Bayes.
...
\mu \sim N(\theta, \tau)
$$
Chúng ta có thể nghĩ đến θ như dự đoán tốt nhất của chúng tôi về chênh lệch số phiếu phổ thông nếu chúng tôi không thấy bất kỳ dữ liệu bỏ phiếu nào và chúng tôi có thể nghĩ ra τ như định lượng mức độ chắc chắn của chúng ta về phỏng đoán này. Nói chung, nếu chúng ta có kiến ​​thức chuyên môn liên quan đến μ, chúng ta có thể cố gắng định lượng nó bằng cách phân phối trước đó. Trong trường hợp thăm dò bầu cử, các chuyên gia sử dụng các nguyên tắc cơ bản , chẳng hạn như tình trạng của nền kinh tế, để phát triển các phân phối trước đó. Dữ liệu được sử dụng để cập nhật dự đoán ban đầu hoặc niềm tin trước đó của chúng tôi . Điều này có thể được thực hiện bằng toán học nếu chúng ta xác định phân bố cho dữ liệu được quan sát, cho bất kỳ μ. Trong ví dụ cụ thể của chúng tôi, chúng tôi sẽ viết ra một mô hình trung bình của các cuộc thăm dò ý kiến, giống như trước đây:
$$
\bar{X} \mid \mu \sim N(\mu, \sigma/\sqrt{N})
$$
Như trước, σ mô tả tính ngẫu nhiên do lấy mẫu và hiệu ứng thăm dò ý kiến. Trong bối cảnh Bayesian, điều này được gọi là phân phối lấy mẫu. Lưu ý rằng chúng tôi viết điều kiện X̅|μ bởi vì μ hiện được coi là một biến ngẫu nhiên.

Chúng tôi không hiển thị các dẫn xuất ở đây, nhưng bây giờ chúng tôi có thể sử dụng Giải tích và một phiên bản của Định lý Bayes để suy ra phân bố của μ có điều kiện của dữ liệu, được gọi là phân phối sau. Đặc biệt chúng ta có thể hiển thị μ|X̅ tuân theo phân phối chuẩn với giá trị kỳ vọng:
$$
\begin{aligned}
...
$$ 
Lưu ý rằng giá trị kỳ vọng là giá trị trung bình có trọng số của dự đoán trước đó của chúng tôi θ và số liệu quan sát được X̅. Trọng số phụ thuộc vào mức độ chắc chắn của chúng ta về niềm tin trước đây của mình, được định lượng bằng τ, và độ chính xác σ/N về bản tóm tắt dữ liệu được quan sát của chúng tôi. Mức trung bình có trọng số này đôi khi được gọi là thu hẹp vì nó thu nhỏ các ước tính về giá trị trước đó.

Những đại lượng này hữu ích cho việc cập nhật niềm tin của chúng tôi. Cụ thể, chúng tôi sử dụng phân phối sau không chỉ để tính giá trị kỳ vọng của μ với dữ liệu được quan sát, nhưng với bất kỳ xác suất nào a chúng ta có thể xây dựng các khoảng, tập trung vào ước tính của chúng ta và với a cơ hội xảy ra.
...
#> [1] 0.0160 0.0401
```
Hơn nữa, giờ đây chúng ta có thể đưa ra tuyên bố xác suất mà chúng ta không thể thực hiện được bằng cách tiếp cận theo chủ nghĩa thường xuyên bằng cách tính toán xác suất hậu nghiệm để Hillary giành được số phiếu phổ thông. Đặc biệt, Pr(μ > 0|X̅) có thể được tính như thế này:
```{r}
...
Giả sử chúng ta đang thu thập dữ liệu từ một người thăm dò ý kiến ​​và cho rằng không có sự thiên vị chung. Người thăm dò ý kiến ​​sẽ thu thập một số cuộc thăm dò với cỡ mẫu là N, vì vậy chúng tôi quan sát một số phép đo mức chênh lệch X1,...,XJ. Giả sử tỷ lệ thực sự của Hillary là p và sự khác biệt là μ. Lý thuyết mô hình chiếc bình cho chúng ta biết rằng các biến ngẫu nhiên này có phân phối chuẩn với giá trị kỳ vọng. μ và sai số chuẩn 2sqrt(p(1-p)/n):
$$
X_j \sim \mbox{N}\left(\mu, \sqrt{p(1-p)/N}\right)
$$ 
Chúng tôi sử dụng chỉ số j để đại diện cho các cuộc thăm dò khác nhau được thực hiện bởi người thăm dò ý kiến ​​này. Đây là mô phỏng cho sáu cuộc thăm dò giả định mức chênh lệch là 2,1 và N là 2.000:
```{r}
set.seed(3)
J <- 6
N <- 2000
...
```
Bây giờ giả sử chúng ta có J = 6 cuộc thăm dò từ mỗi I = 5 những người thăm dò khác nhau. Để đơn giản, giả sử tất cả các cuộc thăm dò đều có cùng cỡ mẫu N. Mô hình bình đựng tro cho chúng ta biết mức phân bổ giống nhau đối với tất cả người thăm dò ý kiến, vì vậy để mô phỏng dữ liệu, chúng tôi sử dụng cùng một mô hình cho mỗi người:
```{r}
I <- 5
J <- 6
N <- 2000
...
```
Để khắc phục điều này, chúng tôi cần biểu thị hai mức độ biến thiên và chúng tôi cần hai chỉ mục, một cho người thăm dò ý kiến ​​và một cho các cuộc thăm dò mà mỗi người thăm dò ý kiến ​​thực hiện. Chúng tôi sử dụng Xij với i đại diện cho người thăm dò ý kiến ​​và j đại diện cho j- cuộc thăm dò thứ từ người thăm dò ý kiến ​​đó. Mô hình hiện đã được tăng cường để bao gồm các hiệu ứng thăm dò ý kiến hi, được FiveThirtyEight gọi là hiệu ứng nhà ở, với độ lệch chuẩn σ(h):
...
\end{aligned}
$$
Để mô phỏng dữ liệu từ một người thăm dò cụ thể, trước tiên chúng ta cần vẽ một hi và tạo dữ liệu thăm dò riêng lẻ sau khi thêm hiệu ứng này. Đây là cách chúng tôi sẽ làm điều đó cho một người thăm dò ý kiến ​​cụ thể. Chúng tôi giả sử σ(h) là 0,025:
```{r}
I <- 5
J <- 6
N <- 2000
...
\end{aligned}
$$
Mô hình này giải thích ba mức độ biến thiên: 1) sự biến thiên trong thành kiến ​​được quan sát từ cuộc bầu cử này sang cuộc bầu cử khác, được định lượng bằng σ(b), 2) sự thay đổi giữa người thăm dò ý kiến ​​hoặc hiệu ứng nhà, được định lượng bằng σ(h) và 3) độ biến thiên của việc lấy mẫu thăm dò ý kiến, mà chúng ta có thể suy ra là sqrt(p(1-p)/N).

Lưu ý rằng không bao gồm một thuật ngữ như b trong các mô hình, chính là nguyên nhân khiến nhiều nhà dự báo trở nên quá tự tin. Biến ngẫu nhiên này thay đổi từ cuộc bầu cử này sang cuộc bầu cử khác, nhưng đối với bất kỳ cuộc bầu cử cụ thể nào, nó giống nhau đối với tất cả những người thăm dò ý kiến ​​và các cuộc thăm dò trong cuộc bầu cử (lưu ý rằng nó không có chỉ mục). Điều này ngụ ý rằng chúng ta không thể ước tính σ(h) với dữ liệu chỉ từ một cuộc bầu cử. Nó cũng ngụ ý rằng các biến ngẫu nhiên Xij cho một năm bầu cử cố định chia sẻ như nhau b và do đó có mối tương quan.

Một cách giải thích b là sự khác biệt giữa mức trung bình của tất cả các cuộc thăm dò từ tất cả những người thăm dò ý kiến ​​và kết quả thực tế của cuộc bầu cử. Bởi vì chúng tôi không biết kết quả thực tế cho đến sau cuộc bầu cử nên chúng tôi không thể ước tính b cho đến sau cuộc bầu cử.
...
one_poll_per_pollster <- polls |> group_by(pollster) |> 
  ...
  ungroup()
```
Ở đây chúng tôi chỉ có một cuộc thăm dò cho mỗi người thăm dò ý kiến ​​nên chúng tôi sẽ bỏ cuộc thăm dò ý kiến j lập chỉ mục và biểu diễn dữ liệu như trước với X1,...,XI. Xin nhắc lại, chúng tôi có dữ liệu từ I = 15 người thăm dò ý kiến. Dựa trên các giả định mô hình được mô tả ở trên, chúng ta có thể chỉ ra một cách toán học rằng giá trị trung bình X̅
```{r}
x_bar <- mean(one_poll_per_pollster$spread)
```
có giá trị kỳ vọng μ, do đó nó cung cấp một ước tính không thiên vị về kết quả quan tâm. Tuy nhiên, ước tính này chính xác đến mức nào? Liệu chúng ta có thể sử dụng độ lệch chuẩn Stample quan sát được để ước lượng sai số chuẩn của X̅?

Hoá ra là vì Xi có mối tương quan với nhau, việc ước tính sai số chuẩn phức tạp hơn những gì chúng tôi đã mô tả cho đến nay. Cụ thể, bằng cách sử dụng các phép tính thống kê nâng cao không được hiển thị ở đây, chúng tôi có thể chỉ ra rằng ước tính phương sai điển hình (bình phương sai số chuẩn)
```{r}
s2 <- with(one_poll_per_pollster, sd(spread)^2 / length(spread))
```
sẽ luôn đánh giá thấp sai số chuẩn thực sự khoảng σ(2,b). Và, như đã đề cập trước đó, để ước tính σ(b), chúng tôi cần dữ liệu từ một số cuộc bầu cử. Bằng cách thu thập và phân tích dữ liệu thăm dò ý kiến ​​từ một số cuộc bầu cử, FiveThirtyEight ước tính mức độ biến động này và nhận thấy rằng σ(b) ~= 0.025. Do đó, chúng tôi có thể cải thiện đáng kể ước tính sai số chuẩn của mình bằng cách thêm đại lượng này:
```{r}
sigma_b <- 0.025
...
```
Để đưa ra các lập luận xác suất, chúng ta sẽ sử dụng mô phỏng Monte Carlo. Đối với mỗi tiểu bang, chúng tôi áp dụng phương pháp Bayesian để tạo ra ngày bầu cử μ. Chúng tôi có thể xây dựng các linh mục cho từng tiểu bang dựa trên lịch sử gần đây. Tuy nhiên, để đơn giản, chúng tôi chỉ định giá trị trước cho mỗi trạng thái giả định rằng chúng tôi không biết gì về điều gì sẽ xảy ra. Vì từ năm bầu cử này sang năm bầu cử khác, kết quả từ một tiểu bang cụ thể không thay đổi nhiều nên chúng tôi sẽ ấn định độ lệch chuẩn là 2% hoặc τ = 0.02. Hiện tại, chúng tôi sẽ giả định một cách không chính xác rằng kết quả thăm dò ý kiến ​​​​từ mỗi tiểu bang là độc lập. Mã cho phép tính Bayes theo các giả định này trông như thế này:
```{r}
mu <- 0
tau <- 0.02
...
Các mô hình mà chúng tôi đã mô tả bao gồm tính biến thiên giữa người thăm dò ý kiến ​​và lỗi lấy mẫu. Nhưng biểu đồ này chỉ dành cho một người thăm dò ý kiến ​​và sự thay đổi mà chúng tôi thấy chắc chắn không phải do lỗi lấy mẫu. Sự biến đổi thêm đến từ đâu? Các đồ thị sau đây chứng minh rõ ràng rằng nó xuất phát từ những biến động theo thời gian không được giải thích bằng lý thuyết giả định p đã được sửa:
...
```
Điều này ngụ ý rằng, nếu chúng ta dự báo, mô hình của chúng ta phải bao gồm một thuật ngữ để giải thích hiệu ứng thời gian. Chúng ta cần viết một mô hình bao gồm một số hạng thiên vị cho thời gian, biểu thị nó b(t). Độ lệch chuẩn của b(t) sẽ phụ thuộc vào t vì chúng ta càng đến gần ngày bầu cử thì số hạng sai lệch này càng gần về 0.

Những người thăm dò ý kiến ​​cũng cố gắng ước tính xu hướng từ những dữ liệu này và kết hợp chúng vào dự đoán của họ. Chúng ta có thể mô hình hóa xu hướng thời gian b(t) với chức năng mượt mà. Chúng ta thường thấy ước tính xu hướng không phải về sự khác biệt mà về tỷ lệ phần trăm thực tế cho mỗi ứng cử viên như thế này:
